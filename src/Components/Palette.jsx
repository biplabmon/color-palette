...
const Palette = () => {
    ...
    // saved new color palette on Localstorage
    useEffect(()=>{
        localStorage.setItem(`myPalette-${id}`, JSON.stringify(myPalette))
    }, [myPalette]);

    const toggleToRgb = event => {
        event.target.value === 'rgb' ? setToRgb("rgb") : setToRgb("hex");
    ...
    const handelFullColorClick = event => {
    ...
    const createColor = () => {
        ...
        if (newColors.length < 20) {
            ...
            alert("You can add only 20 colors to a pallete")
        }
    };

    const handelCopyToClipboard = event => {
        ...
        <PaletteStyled>
            ...
            {toggleColorPicker &&
                <div className="color-picker-con">
                    ...
                    <div className="color-picker-overlay" onClick={() => setToggleColorPicker(!toggleColorPicker)}></div>
                </div>
            }
            <div className="colors">
                {
                    myPalette.colors.map((color, index) => {
                        return <div
                            ...
                            key={index}
                            onClick={(e) => {
                                handelCopyToClipboard(e)
                                handelFullColorClick(e.target.style.backgroundColor)
                            }}
                        >
                            <h4>
